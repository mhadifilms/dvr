...
| Tool | Purpose |
|------|---------|
| `version` | Return dvr / Python / platform version. |
| `doctor` | Diagnose the dvr → Resolve setup (paths, env, process running). Pass `probe=true` to also try a live connection. |
